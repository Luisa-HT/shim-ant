...
const AdminBookingHistoryPage: FC = () => {
    ...
    const handleViewDetails = (record: AdminBookingHistoryDto) => {
        push(`/admin/booking-requests/${record.id_Peminjaman}`)
    };
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md mb-6">
            ...
            <div className="overflow-x-auto"> {/* This div enables horizontal scrolling */}
                <table className="w-full bg-white border border-gray-200 rounded-lg overflow-hidden"> {/* Removed min-w-full from here */}
                    <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[80px]">Booking ID</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[120px]">Item Name</th>
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[120px]">Booked By</th>
                        ...
                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[120px]">Approved By</th>
                    ...
                    {loading ? (
                        ...
                        bookingHistory.map((record) => (
                            <tr key={record.id_Peminjaman} className="hover:bg-gray-50">
                                <td className="px-6 py-4 text-sm text-gray-900">{record.id_Peminjaman}</td>
                                <td className="px-6 py-4 text-sm text-gray-900">{record.nama_Barang}</td>
                                <td className="px-6 py-4 text-sm text-gray-900">{record.nama_Peminjam}</td>
                                {/* Removed whitespace-nowrap from td to allow wrapping */}
                                ...
                                <td className="px-6 py-4 text-sm text-gray-900">{record.nama_Admin || 'N/A'}</td>
